...
enum MazeGenerator {
    static func generate(kind: MazeGeneratorKind, grid: MazeGrid, start: GridPos, rng: inout RandomNumberGenerator) {
        switch kind {
        case .recursiveBacktracker: recursiveBacktracker(grid: grid, start: start, rng: &rng)
        case .prim: prim(grid: grid, start: start, rng: &rng)
        case .wilson: wilson(grid: grid, start: start, rng: &rng)
        }
    }
    ...
    static func recursiveBacktracker(grid: MazeGrid, start: GridPos, rng: inout RandomNumberGenerator) {
        var stack = [start]
    ...
    static func prim(grid: MazeGrid, start: GridPos, rng: inout RandomNumberGenerator) {
        grid.setGenerated(start)
    ...
    static func wilson(grid: MazeGrid, start: GridPos, rng: inout RandomNumberGenerator) {
        grid.setGenerated(start)
        var inMaze = 1
